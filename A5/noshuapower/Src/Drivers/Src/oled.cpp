...
void Oled_::ShowTemperature(float temp)
{
    char buf[16];
...
void Oled_::ShowVoltage(float volt)
{
    char buf[16];
    sprintf(buf, "V:%.2fV", volt);
    ShowString(2, 1, buf);
}

void Oled_::ShowCurrent(float curr)
{
    char buf[16];
    sprintf(buf, "I:%.3fA", curr);
    ShowString(3, 1, buf);
}

void Oled_::ShowBatteryPercent(uint8_t percent)
{
    char buf[16];
    sprintf(buf, "Bat:%d%%", percent);
    ShowString(4, 1, buf);
}
