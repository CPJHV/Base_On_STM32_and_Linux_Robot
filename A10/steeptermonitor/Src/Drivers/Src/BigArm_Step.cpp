...
void BigArm_Step::atim_timx_oc_chy_init(uint16_t arr, uint16_t psc)
{
    BIGARM_TIMX_PWM_CHY_CLK_ENABLE();

    g_atimx_handle.Instance = BIGARM_TIMX_PWM;
    ...
    HAL_TIM_OC_ConfigChannel(&this->g_atimx_handle, &this->g_atimx_oc_chy_handle, BIGARM_TIMX_PWM_CH1);

    HAL_TIM_Base_Start(&this->g_atimx_handle);
}

void BIGARM_TIMX_INT_IRQHandler(void)
{
    HAL_TIM_IRQHandler(&big_arm.g_atimx_handle);
}

void BigArm_Step::change_StarorStop_Monitor(uint8_t para)
{
    if(para == 1)
        HAL_TIM_OC_Start_IT(&this->g_atimx_handle, BIGARM_TIMX_PWM_CH1);
    else
        HAL_TIM_OC_Stop_IT(&this->g_atimx_handle, BIGARM_TIMX_PWM_CH1);
}

void BigArm_Step::set_steper_angle(uint16_t angle, DIR_STATE dir)
{
...
void BigArm_Step::Gpio_Init_other(void)
{
    GPIO_InitTypeDef gpio_init_struct;
    BIGARM_DIR_CLK_ENABLE();
    BIGARM_EN_CLK_ENABLE();
    BIGARM_SPREAD_CLK_ENABLE();
    BIGARM_INDEX_CLK_ENABLE();
    BIGARM_DIAG_CLK_ENABLE();

    gpio_init_struct.Pin = BIGARM_DIR_GPIO_PIN | BIGARM_EN_GPIO_PIN | BIGARM_SPREAD_GPIO_PIN;
    gpio_init_struct.Mode = GPIO_MODE_OUTPUT_PP;
    gpio_init_struct.Pull = GPIO_PULLUP;
    gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(BIGARM_DIR_GPIO_PORT, &gpio_init_struct);

    gpio_init_struct.Pin = BIGARM_DIAG_GPIO_PIN | BIGARM_INDEX_GPIO_PIN;
    gpio_init_struct.Mode = GPIO_MODE_INPUT;
    gpio_init_struct.Pull = GPIO_PULLUP;
    HAL_GPIO_Init(BIGARM_DIAG_GPIO_PORT, &gpio_init_struct);

    this->Dir_State = CWN;
}

void BigArm_Step::Set_Dir(uint8_t para)
{
    if(para == 0){
        HAL_GPIO_WritePin(BIGARM_DIR_GPIO_PORT, BIGARM_DIR_GPIO_PIN, GPIO_PIN_RESET);
        this->Dir_State = CW;
    }else{
        HAL_GPIO_WritePin(BIGARM_DIR_GPIO_PORT, BIGARM_DIR_GPIO_PIN, GPIO_PIN_SET);
        this->Dir_State = CWN;
    }
}

void BigArm_Step::Set_ENABLE(uint8_t para)
{
    if(para == 0)
        HAL_GPIO_WritePin(BIGARM_EN_GPIO_PORT, BIGARM_EN_GPIO_PIN, GPIO_PIN_RESET);
    else
        HAL_GPIO_WritePin(BIGARM_EN_GPIO_PORT, BIGARM_EN_GPIO_PIN, GPIO_PIN_SET);
}

void BigArm_Step::Set_SPREAD(uint8_t para)
{
    HAL_GPIO_WritePin(BIGARM_SPREAD_GPIO_PORT, BIGARM_SPREAD_GPIO_PIN, para ? GPIO_PIN_SET : GPIO_PIN_RESET);
}

uint8_t BigArm_Step::Get_DIAG(void)
{
    return HAL_GPIO_ReadPin(BIGARM_DIAG_GPIO_PORT, BIGARM_DIAG_GPIO_PIN);
}

uint8_t BigArm_Step::Get_INDEX(void)
{
    return HAL_GPIO_ReadPin(BIGARM_INDEX_GPIO_PORT, BIGARM_INDEX_GPIO_PIN);
}

#define MAX_SPEED_TABLE_SIZE 1024
static float s_speed_buffer[MAX_SPEED_TABLE_SIZE];

uint8_t BigArm_Step::calc_speed(int32_t vo, int32_t vt, float time)
{
...
void BigArm_Step::stepmotor_move_rel(int32_t vo, int32_t vt, float AcTime,float DeTime,int32_t step)
{
    ...
    __HAL_TIM_SET_COMPARE(&this->g_atimx_handle,BIGARM_TIMX_PWM_CH1,(uint16_t)(this->g_toggle_pulse/2));
    HAL_TIM_OC_Start_IT(&this->g_atimx_handle,BIGARM_TIMX_PWM_CH1);
    this->Set_ENABLE(1);
}